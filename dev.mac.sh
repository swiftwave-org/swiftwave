...
docker pull ghcr.io/swiftwave-org/swiftwave-dashboard:develop
docker pull haproxytech/haproxy-debian:2.9
docker save -o ./.images/swiftwave-dashboard.tar ghcr.io/swiftwave-org/swiftwave-dashboard:develop
...
docker rm -f swiftwave_dev_env --force > /dev/null 2>&1

echo "Starting swiftwave dev environment..."
# run dev/start.sh script
container_id=$(docker run --privileged -d -v go_cache:/home/user/go/pkg/mod/ -v .:/app:rw -w /app --name swiftwave_dev_env swiftwave_dev_env:latest)

echo "Perform post installation tasks..."
# Run startup script by exec
docker exec swiftwave_dev_env bash -c "cd /app && ./dev/start.sh" | tee -a dev.log
...
echo -e "${YELLOW}docker stop swiftwave_dev_env${NC}"
echo -e "---------------------------------------------------------"
