...
func TestCustomLink(t *testing.T) {
	...
	t.Run("add tcp link", func(t *testing.T) {
		...
		output := "use_backend " + haproxyTestManager.GenerateBackendName(TCPMode, "dummy_backend", 8080)

		// create tcp frontend
		backendName, err := haproxyTestManager.AddBackend(transactionId, TCPMode, "dummy_backend", 8080, 2)
		if err != nil {
			t.Fatal(err)
		}
