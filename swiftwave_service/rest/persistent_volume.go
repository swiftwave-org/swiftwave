...
func (server *Server) downloadPersistentVolumeBackup(c echo.Context) error {
	...
	if persistentVolumeBackup.Type == core.LocalBackup {
		// send file
		filePath := filepath.Join(server.Config.ServiceConfig.DataDir, persistentVolumeBackup.File)
		// file name
		...
		s3config := server.Config.PersistentVolumeBackupConfig.S3Config
		if !s3config.Enabled {
...
func (server *Server) uploadPersistentVolumeRestoreFile(c echo.Context) error {
	...
	filePath := filepath.Join(server.Config.ServiceConfig.DataDir, fileName)
	// Write file
	...
	err = persistentVolumeRestore.Update(c.Request().Context(), *dbTx, server.Config.ServiceConfig.DataDir)
	if err != nil {
		return c.JSON(500, map[string]string{
	...
	if err != nil {
		...
		err = persistentVolumeRestore.Update(c.Request().Context(), server.ServiceManager.DbClient, server.Config.ServiceConfig.DataDir)
		if err != nil {
			log.Println(err)
		}
		return c.JSON(500, map[string]string{
