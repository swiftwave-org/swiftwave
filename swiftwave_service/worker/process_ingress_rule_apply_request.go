...
func (m Manager) IngressRuleApply(request IngressRuleApplyRequest) error {
	...
	if ingressRule.Protocol == core.HTTPSProtocol {
		...
		if ingressRule.Port == 80 {
			...
			err = m.ServiceManager.HaproxyManager.AddTCPLink(haproxyTransactionId, backendName, int(ingressRule.Port), domain.Name, haproxymanager.HTTPMode, m.ServiceConfig.RestrictedPorts)
			if err != nil {
				// set status as failed and exit
				_ = ingressRule.UpdateStatus(ctx, dbWithoutTx, core.IngressRuleStatusFailed)
				deleteHaProxyTransaction(m, haproxyTransactionId)
		...
		err = m.ServiceManager.HaproxyManager.AddTCPLink(haproxyTransactionId, backendName, int(ingressRule.Port), domain.Name, haproxymanager.TCPMode, m.ServiceConfig.RestrictedPorts)
		if err != nil {
			// set status as failed and exit
			_ = ingressRule.UpdateStatus(ctx, dbWithoutTx, core.IngressRuleStatusFailed)
			deleteHaProxyTransaction(m, haproxyTransactionId)
