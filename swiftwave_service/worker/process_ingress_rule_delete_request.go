...
func (m Manager) IngressRuleDelete(request IngressRuleDeleteRequest) error {
	...
	if ingressRule.Protocol == core.HTTPSProtocol {
		...
		if ingressRule.Port == 80 {
			...
			err = m.ServiceManager.HaproxyManager.DeleteTCPLink(haproxyTransactionId, backendName, int(ingressRule.Port), domain.Name, m.ServiceConfig.ServiceConfig.RestrictedPorts)
			if err != nil {
				// set status as failed and exit
