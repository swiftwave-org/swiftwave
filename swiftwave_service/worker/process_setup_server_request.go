...
func (m Manager) setupServerHelper(request SetupServerRequest, ctx context.Context, _ context.CancelFunc) error {
	...
	// check docker socket
	conn, err := ssh_toolkit.NetConnOverSSH("unix", server.DockerUnixSocketPath, 5, server.IP, 22, server.User, m.Config.SystemConfig.SshPrivateKey, 30)
	if err != nil {
	...
	if swarmManagerServer == nil {
		...
		swarmManagerConn, err := ssh_toolkit.NetConnOverSSH("unix", swarmManagerServer.DockerUnixSocketPath, 5, swarmManagerServer.IP, 22, swarmManagerServer.User, m.Config.SystemConfig.SshPrivateKey, 30)
		if err != nil {
